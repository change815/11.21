...
class FCSLoader:
    ...
    def _apply_transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.transform == "minmax":
            LOGGER.debug("Applying Robust Asinh + Percentile Scaling")
            # 1. Asinh 展开
            transformed = np.arcsinh(df / 5.0)

            # 2. 使用分位数 (quantile) 而不是 max
            #    p01 = 1% (底部), p999 = 99.9% (顶部)
            #    这样可以忽略掉 0.1% 的极端噪点
            d_min = transformed.quantile(0.01)
            d_max = transformed.quantile(0.999)

            # 防止除以 0
            # 如果 d_max == d_min (数据完全一样)，就直接返回 0
            ranges = d_max - d_min
            ranges[ranges == 0] = 1.0

            # 3. 归一化并截断 (Clip) 到 0-1 之间
            return ((transformed - d_min) / ranges).clip(0, 1)
