...
if __name__ == "__main__":
    ...
    # 1. 配置日志，否则 LOGGER.info 默认不会在控制台打印
    logging.basicConfig(
        level=logging.INFO,
    ...
    # 2. 定义命令行参数解析器（相当于 C 的 getopt）
    parser = argparse.ArgumentParser(description="Run annotations preparation manually.")
    parser.add_argument("--csv-root", required=True, help="Path to the CSV training data root")
    parser.add_argument("--spec", required=True, help="Path to the annotation spec YAML")
    parser.add_argument("--output", required=True, help="Path to the output gates CSV")
    parser.add_argument("--summary", help="Path to the summary output YAML")
    ...
    # 4. 调用上面定义的业务主函数
    try:
        LOGGER.info("Starting manual annotation preparation...")

        # 调用核心函数
        summary_data = build_gates_from_annotations(
            csv_root=args.csv_root,
            spec_path=args.spec,
            output_csv=args.output
        )

        # 如果指定了 summary 路径，则写入
        if args.summary:
            write_summary(summary_data, args.summary)
