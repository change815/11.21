...
def _normalize_truth_table(
    ...
    *,
    ...
    # ---------------------------------------------------------
    # 🔴 修复点：强制 sample_id 为字符串并补零 (例如 3 -> "003")
    # ---------------------------------------------------------
    if "sample_id" in df.columns:
        # 逻辑：转为字符串，如果是纯数字，则补足3位；如果本来就是 sample_003 这种则不动
        df["sample_id"] = df["sample_id"].astype(str).apply(lambda x: x.zfill(3) if x.isdigit() else x)
